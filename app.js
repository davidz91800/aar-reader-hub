...
const REPORTS_SNAPSHOT_KEY = "aar_reader_reports_snapshot_v1";
const FILTERS_STATE_KEY = "aar_reader_filters_state_v1";
const AUTO_RESYNC_MIN_INTERVAL_MS = 300000;
const AUTO_RESYNC_TICK_MS = 300000;
const DRIVE_ERROR_COOLDOWN_MS = 10 * 60 * 1000;
const DRIVE_COOLDOWN_KEY = "aar_reader_drive_cooldown_until_v1";
const SHOW_PENDING_QWI_REVIEW = false;
...
function deriveMeta(a) {
  ...
  const redacteur = identityAnonymized ? "Anonymise" : ([rank, name].filter(Boolean).join(" ").trim() || "N/A");
...
function filtered() {
  ...
  if (q) {
    rows = rows.filter((r) => [
      r.title, isRecordAnonymized(r) ? "Anonymise" : r.redacteur, isRecordAnonymized(r) ? "" : r.nom, isRecordAnonymized(r) ? "" : r.prenom, isRecordAnonymized(r) ? "" : r.unit,
      r.reportKind,
...
function renderList() {
  ...
  el.aarGrid.innerHTML = rows.map((r) => {
    ...
    const metaParts = [anonymized ? "Anonymise" : r.redacteur];
...
function openDetail(id) {
  ...
  const redacteur = anonymized
    ? (["Anonymise", r.fleet].filter(Boolean).join(" | ") || "Anonymise")
